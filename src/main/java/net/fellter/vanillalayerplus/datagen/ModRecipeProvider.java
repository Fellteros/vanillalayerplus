...
//                layerBlockRecipe(ModBlocks.CHISELED_TUFF_BRICKS_LAYER, Blocks.CHISELED_TUFF_BRICKS).offerTo(exporter);
                ...
                Registries.BLOCK.forEach(block -> {
                    if (ModBlocks.DATAGEN_ARGS.containsKey(block)) {
                        ItemConvertible ingredient = ModBlocks.DATAGEN_ARGS.get(block).recipeIngredient;
                        if (Registries.BLOCK.getId(block).getNamespace().equals(VanillaLayerPlus.MOD_ID) && block instanceof LayerBlock && ingredient != null) {
                            layerBlockRecipe(block, ingredient).offerTo(exporter);
                        }
                    }
                });
